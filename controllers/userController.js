...
const createUser = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "User is successfully registered.", users: [...users] });
  } catch (error) {
...
const updateUser = async (req, res) => {
  try {
    ...
    const updateUser = await User.findOneAndUpdate(
      {Id},
      { username, email, password, gender, terms, hobbies, state, address },
      { new: true }
    );
    console.log(Id, req?.body, updateUser, "Id");
    if (!updateUser) {
